...
public class Client extends Application implements Initializable {
		...
		if (protocol.equals("LogIn")) {
			...
			Platform.runLater(() -> {
				for (int i = 0; i < msg.size(); i++) {
					...
					RoomItem item = new RoomItem(_msg.getKey().getKey(), _msg
							.getKey().getValue(), _msg.getValue());
					lbbgui.getRmlst().add(item);
				}
				lbbgui.getRmlst_tv().setItems(lbbgui.getRmlst());
			});
			...
			Platform.runLater(() -> {
				...
				lbl.setStyle("-fx-text-fill: white;"
						...
						+ "-fx-background-color: linear-gradient(to bottom, rgb(182,232,251), rgb(27,183,241));"
						+ "-fx-border-radius: 5; -fx-border-width: 5; "
						+ "-fx-border-color: linear-gradient(to bottom, rgb(182,232,251), rgb(27,183,241));");
			...
			Platform.runLater(() -> {
				...
				lbl.setStyle("-fx-text-fill: white;"
						...
						+ "-fx-border-radius: 5; -fx-border-width: 5; "
						+ "-fx-border-color: linear-gradient(to bottom, rgb(182,232,251), rgb(27,183,241));");
				String _image = RoomGUI.class.getResource(
						"imoticon/" + msg.getValue() + ".png").toExternalForm();
	...
	class LoginGUI extends Stage implements Initializable {
		...
		public LoginGUI() {
			...
			k2nm_lbl = new Label("K2NMchat");
			k2nm_lbl.setId("k2nm_lbl");
			k2nm_lbl.setLayoutX(25.0);
			k2nm_lbl.setLayoutY(15.0);
			k2nm_lbl.setPrefSize(350.0, 50.0);
			root.setTopAnchor(k2nm_lbl, 15.0);
			root.getChildren().add(k2nm_lbl);

			id_lbl = new Label("ID");
			id_lbl.setId("id_lbl");
			id_lbl.setLayoutX(25.0);
			id_lbl.setLayoutY(100.0);
			id_lbl.setPrefSize(130.0, 25.0);
			root.setLeftAnchor(id_lbl, 25.0);
			root.getChildren().add(id_lbl);

			ip_lbl = new Label("Server IP");
			ip_lbl.setId("ip_lbl");
			ip_lbl.setLayoutX(25.0);
			ip_lbl.setLayoutY(140.0);
			ip_lbl.setPrefSize(130.0, 25.0);
			root.setLeftAnchor(ip_lbl, 25.0);
			root.getChildren().add(ip_lbl);

			port_lbl = new Label("Server port");
			port_lbl.setId("port_lbl");
			port_lbl.setLayoutX(25.0);
			port_lbl.setLayoutY(180.0);
			port_lbl.setPrefSize(130.0, 25.0);
			root.setLeftAnchor(port_lbl, 25.0);
			root.getChildren().add(port_lbl);

			idInput = new TextField();
			idInput.setId("idInput");
			idInput.setLayoutX(110.0);
			idInput.setLayoutY(100.0);
			idInput.setPrefSize(200.0, 25.0);
			idInput.setPromptText("length of ID: 2 ~ 20");
			root.setRightAnchor(idInput, 25.0);
			root.getChildren().add(idInput);

			ipInput = new TextField();
			ipInput.setId("ipInput");
			ipInput.setLayoutX(110.0);
			ipInput.setLayoutY(140.0);
			ipInput.setPrefSize(200.0, 25.0);
			ipInput.setPromptText("0.0.0.0 ~ 255.255.255.255");
			root.setRightAnchor(ipInput, 25.0);
			root.getChildren().add(ipInput);

			portInput = new TextField();
			portInput.setId("portInput");
			portInput.setLayoutX(110.0);
			portInput.setLayoutY(180.0);
			portInput.setPrefSize(200.0, 25.0);
			...
			btnLogin.setLayoutX(155);
			btnLogin.setLayoutY(245);
		...
		public void handlebntLogin(ActionEvent event) {
			...
			try {
				...
				if (userID.length() < 2 || 20 < userID.length())
	...
	class LobbyGUI extends Stage implements Initializable {
		...
		public LobbyGUI() {
			...
			rmlst_lbl = new Label("Room List");
			rmlst_lbl.setId("rmlst_lbl");
			rmlst_lbl.setLayoutX(30.0);
			rmlst_lbl.setLayoutY(14.0);
			rmlst_lbl.setPrefSize(500.0, 50.0);
			root.setRightAnchor(rmlst_lbl, 30.0);
			root.setTopAnchor(rmlst_lbl, 10.0);
			root.getChildren().add(rmlst_lbl);

			rmlst_tv = new TableView();
			rmlst_tv.setId("rmlst_tv");
			rmlst_tv.setLayoutX(14.0);
			rmlst_tv.setLayoutY(14.0);
			rmlst_tv.setPrefSize(500.0, 420.0);
			root.setLeftAnchor(rmlst_tv, 30.0);
			root.setTopAnchor(rmlst_tv, 70.0);
			...
			rmType_col = new TableColumn("Room Type");
			rmType_col.setId("rmType");
			...
			rmType_col
					.setCellValueFactory(new PropertyValueFactory<RoomItem, String>(
							"roomType"));
			rmName_col = new TableColumn("Room Name");
			rmName_col.setId("rmName");
			...
			rmName_col
					.setCellValueFactory(new PropertyValueFactory<RoomItem, String>(
							"roomName"));
			usrNum_col = new TableColumn("User Num");
			usrNum_col.setId("usrNum_col");
			usrNum_col.setPrefWidth(50.0);
			usrNum_col.setResizable(false);
			usrNum_col.setEditable(false);
			usrNum_col
					.setCellValueFactory(new PropertyValueFactory<RoomItem, Integer>(
							"userNum"));
			rmlst_tv.getColumns().addAll(rmType_col, rmName_col, usrNum_col);
			rmlst = FXCollections.observableArrayList();

			usrlst_lbl = new Label("User List");
			usrlst_lbl.setId("usrlst_lbl");
			usrlst_lbl.setLayoutX(739.0);
			usrlst_lbl.setLayoutY(14.0);
			usrlst_lbl.setPrefSize(200.0, 50.0);
			root.setRightAnchor(usrlst_lbl, 30.0);
			root.setTopAnchor(usrlst_lbl, 10.0);
			root.getChildren().add(usrlst_lbl);

			usrlst_lv = new ListView<String>();
			usrlst_lv.setId("usrlst_lv");
			usrlst_lv.setLayoutX(435.0);
			usrlst_lv.setLayoutY(50.0);
			usrlst_lv.setPrefSize(200.0, 420.0);
			root.setRightAnchor(usrlst_lv, 30.0);
			root.setTopAnchor(usrlst_lv, 70.0);
			root.getChildren().add(usrlst_lv);
			usrlst = FXCollections.observableArrayList();

			btnEnterRm = new Button("Enter Room");
			btnEnterRm.setId("btnEnterRm");
			btnEnterRm.setLayoutX(30.0);
			btnEnterRm.setLayoutY(474.0);
			btnEnterRm.setPrefSize(200.0, 50.0);
			root.setBottomAnchor(btnEnterRm, 30.0);
			root.setLeftAnchor(btnEnterRm, 30.0);
			btnEnterRm.setOnAction(event -> handlebtnEnterRm(event));
			root.getChildren().add(btnEnterRm);

			btnMakeRm = new Button("Make Room");
			btnMakeRm.setId("btnMakeRm");
			btnMakeRm.setLayoutX(239.0);
			btnMakeRm.setLayoutY(474.0);
			btnMakeRm.setPrefSize(200.0, 50.0);
			btnMakeRm.setOnAction(event -> handlebtnMakeRm(event));
			root.setBottomAnchor(btnMakeRm, 30.0);
			root.setLeftAnchor(btnMakeRm, 250.0);
			root.getChildren().add(btnMakeRm);

			btnLogout = new Button("Log Out");
			btnLogout.setId("btnLogout");
			btnLogout.setLayoutX(570.0);
			btnLogout.setLayoutY(474.0);
			btnLogout.setPrefSize(200.0, 50.0);
			btnLogout.setOnAction(event -> handlebtnLogout(event));
			root.setBottomAnchor(btnLogout, 30.0);
			root.setRightAnchor(btnLogout, 30.0);
		...
		void handlebtnMake(ActionEvent event4) {
			...
			if (protocol.equals("MakePrivate")) {
				if (new PrivateRoom().checkRoomName(name)
						&& new PrivateRoom().checkSettingPW(pw))
	...
	class RoomGUI extends Stage implements Initializable {
		...
		public RoomGUI() {
			...
			vb1.setLayoutX(14.0);
			vb1.setLayoutY(14.0);
			vb1.setPrefSize(550.0, 200.0);
			vb1.setSpacing(15.0);
			root.setBottomAnchor(vb1, 20.0);
			root.setLeftAnchor(vb1, 20.0);
			root.setTopAnchor(vb1, 20.0);
			root.getChildren().add(vb1);

			msgScrl = new ScrollPane();
			msgScrl.setId("msgScrl");
			...
			HBox _hb1 = new HBox();
			_hb1.setPrefSize(550.0, 90.0);
			_hb1.setSpacing(10.0);
			...
			btnSndMsg = new Button("Send");
			btnSndMsg.setId("btnSndMsg");
			btnSndMsg.setPrefSize(60.0, 30.0);
			btnSndMsg.setOnAction(event -> handleBtnSndMsg(event));
			_hb1.getChildren().add(btnSndMsg);

			HBox _hb2 = new HBox();
			_hb2.setPrefSize(200.0, 200.0);
			_hb2.setSpacing(10.0);
			vb1.getChildren().add(_hb2);

			ScrollPane sp = new ScrollPane();
			sp.setPrefSize(480.0, 200.0);
			sp.setPrefViewportHeight(200.0);
			sp.setPrefViewportWidth(480.0);
			...
			for (int i = 0; i < imtcNum; i++) {
				...
				String image = RoomGUI.class.getResource(
						"imoticon/" + name + ".png").toExternalForm();
				btn[i] = new Button();
				btn[i].setPrefSize(40.0, 40.0);
				btn[i].setStyle("-fx-background-image: url(" + image + ");"
			...
			btnSndImtc = new Button("Send");
			btnSndImtc.setId("btnSndImtc");
			btnSndImtc.setPrefSize(60.0, 30.0);
			btnSndImtc.setOnAction(event -> handleSndImtcBtnAction(event));
			_hb2.getChildren().add(btnSndImtc);

			VBox vb2 = new VBox();
			vb2.setLayoutX(580.0);
			vb2.setLayoutY(20.0);
			vb2.setSpacing(15.0);
			root.setBottomAnchor(vb2, 20.0);
			root.setRightAnchor(vb2, 20.0);
			root.setTopAnchor(vb2, 20.0);
			...
			info_lbl = new Label("Info");
			info_lbl.setId("info_lbl");
			...
			usrlst_view = new ListView();
			usrlst_view.setId("usrlst_view");
			...
			btnSndFile = new Button("Send File");
			btnSndFile.setId("btnSndFile");
			btnSndFile.setPrefSize(200.0, 50.0);
			btnSndFile.setOnAction(event -> handleSndFileBtnAction(event));
			_vb3.getChildren().add(btnSndFile);

			btnExitRm = new Button("Exit Room");
			btnExitRm.setId("btnExitRm");
		...
		private void handleBtnSndMsg(ActionEvent event) {
			...
			if (!msg.isEmpty()) {
				...
				lbl.setStyle("-fx-text-fill: white;"
						+ "-fx-background-radius: 5; "
						+ "-fx-background-color: linear-gradient(to bottom, rgb(146,200,230), rgb(12,112,169));"
						+ "-fx-border-radius: 5; -fx-border-width: 5; "
						+ "-fx-border-color: linear-gradient(to bottom, rgb(146,200,230), rgb(12,112,169));");
		...
		private void handleSndImtcBtnAction(ActionEvent e) { // when send
																// imoticon
		...
		private void handleSndFileBtnAction(ActionEvent e) {
			System.out.println("FileSnd");
			send("FileRequest", usrlst_view.getSelectionModel()
					.getSelectedItem());
		}
