...
public class Client extends JFrame implements ActionListener {
	...
	private void inmessage(String str) // 서버로부터 들어오는 모든 메세지
	{
		...
		if (protocol.equals("NewUser")) {
			...
			System.out.println(user + "사용자부터 받은 메세지 " + note);

			JOptionPane.showMessageDialog(null, note, user + "님으로 부터 쪽지",
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == login_btn) {
			...
			Network();
		} else if (e.getSource() == notesend_btn) { // 쪽지보내기
			System.out.println("note send button click");
			String user = (String) User_list.getSelectedValue(); // 유저 이름획득

			String note = JOptionPane.showInputDialog("보낼메시지"); // 팝업창띄우고
